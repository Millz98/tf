    ...
    if unchanged:
        for file in unchanged:
            ...
            if file_stat_new[0][1] - file_stat_old[0][1] != 0 or file_stat_new[0][2] - file_stat_old[0][2] != 0:
                table.append(
                    [file_stat_new[0][0], file_stat_new[0][1], file_stat_new[0][1] - file_stat_old[0][1],
                     file_stat_new[0][2], file_stat_new[0][2] - file_stat_old[0][2]])
    return table
...
if __name__ == "__main__":
    if len(sys.argv) == 3:
        ...
        try:
            ...
            print("### Changes")
            print("```")
            print(tabulate([headers] + sorted(table, key=lambda x: -x[1]), headers="firstrow",
                           intfmt=(..., "d", "+d"), floatfmt=(..., ..., ..., ".1f", "+.1f")) + "\n")
        ...
        try:
            ...
            for dir_name, group in itertools.groupby(
                    sorted([(x[0].rsplit("/", 1)[0], x[1], x[2]) for x in table]), key=lambda x: x[0]):
